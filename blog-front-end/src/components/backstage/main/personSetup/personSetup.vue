    ...
    export default {
        setup(props) {
            ...
            const state = reactive({
                ...
                clearCount: "",
                // 图片上传到服务器的路径
                uploadURL: process.env.VUE_APP_URL + "upload/userIcon",
                // 服务器路径
                ...
                if (state.basic.userName !== state.userInfo.userName ||
                    ...
                    basicFormRef.value.validate((valid) => {
                        ...
                        if (valid) {
                            ...
                            // 将修改信息传给服务器
                            axios.post("/user/updateUserBasicsInfo",
                                params
                            ).then((res) => {
                                if (res.data.code == 0) {
                                    // 自动重新登录一次 修改token的值
                ...
                toBindFormRef.value.validateField('email', err => {
                    ...
                    if (err || type == 1) {
                        ...
                        const params = {
                            ...
                            content: "【ToLZB博客】验证码：" + state.toBind.verificationCode +
